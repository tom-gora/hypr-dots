...
if [[ ! $(pidof rofi) ]]; then
	...
	if [[ "$ICON" != "-1" && "$PROMPT" != "-1" ]]; then
		...
		if ! RESULT=$("$BIN" menu --menu-config "$MENU_CONFIG_PATH" | rofi -dmenu "${FULL_ROFI_ARGS[@]}"); then
			exit 1
		fi
		"$BIN" menu --menu-config "$MENU_CONFIG_PATH" --result "$RESULT"
	elif [[ "$ICON" != "-1" ]]; then
		ICON_ROFI_ARGS=("${ROFI_ARGS_FOR_ROFI[0]}")
		if ! RESULT=$("$BIN" menu --menu-config "$MENU_CONFIG_PATH" | rofi -dmenu "${ICON_ROFI_ARGS[@]}"); then
			exit 1
		fi
		"$BIN" menu --menu-config "$MENU_CONFIG_PATH" --result "$RESULT"
	elif [[ "$PROMPT" != "-1" ]]; then
		PROMPT_ROFI_ARGS=("${ROFI_ARGS_FOR_ROFI[1]}")
		if ! RESULT=$("$BIN" menu --menu-config "$MENU_CONFIG_PATH" | rofi -dmenu "${PROMPT_ROFI_ARGS[@]}"); then
			exit 1
		fi
		"$BIN" menu --menu-config "$MENU_CONFIG_PATH" --result "$RESULT"
	else
		if ! RESULT=$("$BIN" menu --menu-config "$MENU_CONFIG_PATH" | rofi -dmenu); then
			exit 1
		fi
		"$BIN" menu --menu-config "$MENU_CONFIG_PATH" --result "$RESULT"
	fi
